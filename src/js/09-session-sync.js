...
/** In-page replacement for window.prompt() — same reasoning as
 *  _showConfirmModal above. Resolves the entered text (trimmed by the
 *  caller), or null if cancelled, matching prompt()'s own behavior. */
function _showPromptModal(message, defaultValue) {
  return new Promise(resolve => {
    const backdrop = document.createElement('div');
    backdrop.style.cssText = 'position:fixed;inset:0;z-index:2147483000;background:rgba(0,0,0,.6);display:flex;align-items:center;justify-content:center;';
    const box = document.createElement('div');
    box.style.cssText = 'background:#1c140f;color:#e9ddc8;border:1px solid #6b5842;border-radius:6px;padding:20px 24px;max-width:380px;width:90%;box-shadow:0 8px 30px rgba(0,0,0,.6);font:14px/1.5 inherit;';
    const msg = document.createElement('div');
    msg.style.cssText = 'white-space:pre-wrap;margin-bottom:10px;';
    msg.textContent = message;
    const input = document.createElement('input');
    input.type = 'text';
    input.value = defaultValue || '';
    input.style.cssText = 'width:100%;box-sizing:border-box;padding:6px 8px;margin-bottom:16px;background:#2a221a;color:#e9ddc8;border:1px solid #6b5842;border-radius:4px;font:inherit;';
    const row = document.createElement('div');
    row.style.cssText = 'display:flex;justify-content:flex-end;gap:10px;';
    ...
    row.appendChild(mkBtn('Cancel', false, () => finish(null)));
    row.appendChild(mkBtn('OK', true, () => finish(input.value)));
...
/** Populate _playerName once, via the modal above instead of
 *  window.prompt() — same Electron dialog-input-lock risk as confirm().
 *  Only ever needed the first time a player joins with no name set yet;
 *  every other call to getMyPlayerName() elsewhere in the app
 *  (07-combat-window.js, this file's own vitals/command code) just
 *  reads the cached value this sets, so nothing else needs to change. */
async function _ensurePlayerName() {
  ...
  const name = await _showPromptModal('Enter your player name (displayed on the battlefield):');
  if (name && name.trim()) {
