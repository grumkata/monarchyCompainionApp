...
function onClick(e) {
  ...
  /* THERE IS NO LIGHT THEME, so there was no business having a switch for
     ...
     in that row — it is in the hall, and you are not. */
  if (act === 'settings') { shut(); root.Shell.backToHall();
                            return setTimeout(() => root.Menu && root.Menu.take('set'), 60); }

  if (act === 'host') return startHosting();
  if (act === 'close' || act === 'leave-live') {
    return S().leave().then(() => { draw(); });
  }
}
...
function show() { mount(); said = ''; draw(); el.hidden = false; open = true;
...
root.addEventListener('monarchy:session', () => { if (open) draw(); });
