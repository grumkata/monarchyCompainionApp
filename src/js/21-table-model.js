...
const Content = root.TableContent ||
  (typeof require !== 'undefined' ? require('./20-table-content.js') : null);
...
const T = {
  ...
  /* ── the bin is a bin ────────────────────────────────────────
     It keeps what you put in, newest first, and you can reach
     back in. Nobody discovers a confirm dialog mid-session.     */
  bin(id) {
    ...
    this.state.bin.unshift(t);
    if (this.state.bin.length > BIN_KEEPS) this.state.bin.length = BIN_KEEPS;
